...
pub fn load(path: &Path) -> Result<Values> {
    let text =
...
pub fn save(path: &Path, values: &Values) -> Result<()> {
    ...
    let body = serde_yaml::to_string(&LocalFile { values })?;
    out.push_str(body.trim_start_matches("---\n").trim_end());
    out.push('\n');
    std::fs::write(path, out).with_context(|| format!("writing {}", path.display()))
}

fn parse_str(text: &str) -> Result<Values> {
    ...
    if let Ok(with) = serde_yaml::from_value::<WithValues>(doc.clone()) {
